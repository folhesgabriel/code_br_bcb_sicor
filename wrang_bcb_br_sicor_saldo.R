...
dir_create_ano <- function(ano){
  ...
  dir.create(paste0("~/Template Dados/tmp/microdados_saldo/ano=",ano),
             showWarnings = FALSE,
             recursive = T)
}
dir_create_mes <- function(mes){
  
  dir.create(paste0("~/Template Dados/tmp/microdados_saldo/ano=",ano,"/mes=",mes),
             showWarnings = FALSE,
             recursive = T)
}
...
arquivos_brutos_saldo <- list.files("~/Template Dados/input",
                                    full.names = T,
                                    pattern = 'SALDO')
  ...
  for(ano in unique(file$ANO_BASE)){
    ...
    for(mes in unique(file$MES_BASE)){
      ...
      write.table(file_particionado,
                  paste0("~/Template Dados/tmp/microdados_saldo/ano=",ano,"/mes=",mes,"/microdados_saldo_",ano,"_",mes,z,".csv"),
                  na = '',
                  quote = FALSE,
                  row.names = FALSE,
                  sep = ",")
